    ...
    try {
        ...
        if ($count -gt 0) {
            ...
            $SearchResult.Updates | ForEach-Object {
                $i++
                if ($i -le 10) {
                    $title = if ($_.Title.Length -gt 70) {
                        "$($_.Title.Substring(0, 70))..."
                    } else {
                        $_.Title
                    }
        ...
        Write-Host "`n  Unable to query Windows Update." -ForegroundColor Yellow
...
function Show-InstallRestartMenu {
    ...
    if (-not (Test-AdminRights)) {
        if (Get-Command Request-Elevation -ErrorAction SilentlyContinue) {
    ...
    try {
        ...
        if ($downloadResult.ResultCode -ne 2) {
            Write-Host "  Download finished with result code $($downloadResult.ResultCode)." -ForegroundColor Yellow
        ...
        Write-ASALog ("Updates install result code: {0}" -f $installResult.ResultCode) -Level Info
